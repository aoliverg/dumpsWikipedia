...
parser.add_argument('-c','--categories', action="store", dest="categories", help='A file with one category per line.',required=True)
            ...
            for revision in page:
                ...
                if any(category in valid_categories for category in categories):
                    ...
                    for linia in linies:
                        ...
                        if not linia.startswith(category_namespaces[language]) and not linia.startswith("|") and not linia.startswith("<") and len(linia)>0:
                            print(linia)
                            sortida.write(linia+"\n")
                    sortida.close()
